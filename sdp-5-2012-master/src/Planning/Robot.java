...
public class Robot extends ObjectDetails {
	...
	private NXTInfo info = new NXTInfo(NXTCommFactory.BLUETOOTH, "NXT-G2",
			"00:16:53:08:DF:AB");	
...
//			 System.out.println("STACK CLEARED");
	...
	public void dribble(int distance){
		
		int command = DRIBBLE|(distance << 8);
		
		System.out.println("Dribble distance of " + distance + " cm");
		addCommand(command);
		
	}
	
	public void navigate(int distance, int angle){
		
		int command = NAVIGATE|(distance << 8)|(angle << 8);
		
		System.out.println("Turn angle " + angle + " and travel a distance of " + distance);
		addCommand(command);
