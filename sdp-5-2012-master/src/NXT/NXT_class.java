...
public class NXT_class implements Runnable{
	...
	private static final float TRACK_WIDTH = (float) 5.0394; // Secondary table
	private static final float WHEEL_DIAMETER = (float) 5.3543;
	...
	private final static int DRIBBLE =0X0D;
	private final static int NAVIGATE =0X0E;
	
	
	public static void main(String[] args) throws Exception {

		DifferentialPilot pilot = new DifferentialPilot(WHEEL_DIAMETER,
				TRACK_WIDTH, Motor.C, Motor.B, false);
			...
			try {
				...
				while (n != QUIT) {
					...
					case ROTATE:	
						int rotateAngle = n >> 8;
						LCD.clear();
						LCD.drawString("angle: " + rotateAngle, 0, 2);
						LCD.refresh();
					...
					case DRIBBLE: //moves forwards
						int dribbleDist = n >> 8;
						LCD.clear();
						LCD.drawString("dribbleDist = " + dribbleDist, 0, 2);
						LCD.refresh();	
						UltrasonicSensor dribbler = new UltrasonicSensor(SensorPort.S3);
						/*I2CPort MULTIPLEXER_PORT = SensorPort.S2;
					    byte dribble1Direction = 1;
					    byte dribble1Speed = 2;
					    byte dribble2Direction = 5;
					    byte dribble2Speed = 6;
					    byte MULTIPLEXER_ADDRESS = (byte) 0xB4;
					    byte MULTIPLEXER_OFF = 0;
					    byte MULTIPLEXER_FORWARDS = 1;
					    byte MULTIPLEXER_BACKWARDS = 2;
					    byte MULTIPLEXER_MAX_SPEED = (byte) 255;
					    I2CSensor multiplexer;
				        MULTIPLEXER_PORT.i2cEnable(I2CPort.STANDARD_MODE);
					    multiplexer = new I2CSensor(MULTIPLEXER_PORT);
					    multiplexer.setAddress(MULTIPLEXER_ADDRESS);
					    multiplexer.sendData(dribble1Direction, MULTIPLEXER_BACKWARDS);
				        multiplexer.sendData(dribble1Speed, MULTIPLEXER_MAX_SPEED);
				        multiplexer.sendData(dribble2Direction, MULTIPLEXER_BACKWARDS);
				        multiplexer.sendData(dribble2Speed, MULTIPLEXER_MAX_SPEED);*/
						pilot.setAcceleration(100);
				        pilot.setTravelSpeed(200);
				        pilot.travel(dribbleDist);
				        break;
					...
					case NAVIGATE:
						...
						int navAngle = n >> 8;
						LCD.clear();
						LCD.drawString("navDist = " + navDist + "navAngle = " + navAngle, 0, 2);
						LCD.refresh();
						pilot.setRotateSpeed(pilot.getRotateMaxSpeed()/5);
						pilot.rotate(navAngle, false);
						pilot.setAcceleration(100);
				        pilot.setTravelSpeed(200);
				        pilot.travel(navDist);
